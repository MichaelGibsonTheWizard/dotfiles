(function () {
  ...
  function handleImg() {
    ...
    for (i = 0; i < m.length; i = i + 1) {
      ...
      if (m[i].currentSrc) {
        ...
        mod = {
          description: getDescription(m[i]),
          sourceOrder: order
        };
        order = order + 1;
        ...
        if (v) {
          mod.pinMedia = v;
          // reload
          loadImg({ src: v, mod: mod, update: m[i].currentSrc });
        } else {
          // load whatever is currently on the screen
          loadImg({ src: m[i].currentSrc, mod: mod });
        }
        // be nice to Twitter
        if ($.v.here.match($.a.pattern.page.twitter) && m[i].currentSrc.match($.a.pattern.img.twitter)) {
          a = m[i].parentNode;
          while (a.tagName) {
  ...
  function process() {
    ...
    if ($.v.insta) {
      ig = {
        reason: "insta_found",
        extras: { media_id: $.v.insta.id, owner_id: $.v.insta.owner }
      };
      if ($.v.insta.hashtags) {
